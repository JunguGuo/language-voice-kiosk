...
export default function LanguagePick(){
  ...
  useEffect(()=>{ getLanguages().then(setLangs); },[]);

  function proceed(){
    const code = langs[idx]?.code;
    if (code) { setLanguage(code); nav('/scenarios'); }
  }
    ...
    <div className="h-screen grid place-items-center px-8">
      <div className="w-full max-w-3xl grid md:grid-cols-[1fr_auto] gap-8 items-start">
        <div className="card p-0">
          {langs.map((l,i)=>(
            <button key={l.code} onClick={()=>setIdx(i)} className="w-full text-left">
        ...
        <div className="space-y-6">
          ...
          <button onClick={proceed} className="btn btn-primary">I’m ready →</button>
        </div>
      </div>
    </div>
    </Page>
